...
import CTableWrapper from "../base/HistoryTable.vue";
...
export default {
  ...
  // },
  created () {           // 초기화 함수를 정의 한다.
    axios.get('https://7g3u095i99.execute-api.us-east-1.amazonaws.com/product/history')
      .then(response =>  {
      ...
      for (let i= 0;i<=this.toDoItems.length -1;i++) {
        arr.push(
            { id : this.toDoItems[i][0],
              datetime : this.toDoItems[i][1],
              sitename : this.toDoItems[i][2],
              schedulername : this.toDoItems[i][3],
              keywordname : this.toDoItems[i][4],
              keyword : this.toDoItems[i][5],
              cnt : this.toDoItems[i][6],
              pmid : this.toDoItems[i][7]})
      }
      return arr;
    }
  }
